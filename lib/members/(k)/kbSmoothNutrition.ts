import { MembersListType } from './../membersListType'

export const kbSmoothNutrition : MembersListType = {
    memberStatus: false,
    id : "kb-smooth-nutrition",
    businessName : "K & B's Smooth Nutrition",
    bannerImage : "",
    profileLogo : "kbs-smooth-nutrition",
    profileDescription : "K & B’s Smooth Nutrition offers organic, nutrient-packed superfood smoothies with grab‑and‑go and delivery options in Memphis and Cordova. Founded by health enthusiast Sondra King, the company empowers customers with wholesome blends to support wellness and vitality.",
    profileCategory : [
        "food-dining-hospitality",
        "retail-shopping-specialty-stores"
    ],
    profileSubCategory : [
        "fast-food-takeaways",
        "coffee-shops-cafes",
        "online-retailers",
        "beverages-smoothies"
    ],
    categoryTag: [],
    profileBusinessDescription : `<p>K & B’s Smooth Nutrition, founded by entrepreneur and author Sondra King, is a Memphis and Cordova-based organic smoothie bar committed to holistic wellness. Combining superfood ingredients and fresh produce, their nutrient-rich blends are designed to energize and support optimal health for everyday life.</p>
                                    <p>Customers can order online for delivery or enjoy the convenience of multiple grab‑and‑go locations across the city. Whether you're refueling post-workout or looking for a wholesome snack, K & B’s caters to busy lifestyles without compromising quality.</p>
                                    <p>Driven by a mission rooted in healthy living, Sondra's philosophy is inspired by a blend of entrepreneurship and holistic nutrition—designed to empower others toward longevity and well‑being. A recurring tagline reinforces this commitment: “Blending to better health!”</p>
                                    <p>The brand has a growing local presence online and via Instagram, where they connect with wellness communities and feature promotions like 10% off new orders through email sign-ups. Customer reviews highlight their “just fruit” purity and thoughtful portion sizing.</p>
                                    <p>With a BBB rating of A, K & B’s Smooth Nutrition is recognized for professional practices since incorporation in February 2023. Sondra King leads customer engagement and operations, reflecting the close-knit nature of this business.</p>`,
    gallery : [],
    businessHours : [
        { day : "Monday", hours : "8am - 5pm"},
        { day : "Tuesday", hours : "8am - 5pm"},
        { day : "Wednesday", hours : "8am - 5pm"},
        { day : "Thursday", hours : "8am - 5pm"},
        { day : "Friday", hours : "8am - 5pm"},
        { day : "Saturday", hours : "9am - 4pm"},
        { day : "Sunday", hours : "Closed"},
    ],
    addressLink : "https://maps.app.goo.gl/frmwj2VTSh4jRnHCA",
    address : "588 Cooper St, Memphis, TN 38104, United States",
    email : "sondra@kbssmoothnutrition.com",
    website : "https://kbssmoothnutrition.com/",
    telNumber : "9016136141",
    facebook : "",
    instagram : "https://www.instagram.com/kbssmoothnutrit?igsh=MTRudnA0dTR2ajRtZg%3D%3D&utm_source=qr",
    youtube : "",
    linkedIn : "",
    linktree : "",
    twitter: "https://www.x.com/kbssmoothnutrit?s=21&t=Nz1PVJVcCsw_md4BFM4EYg",
    tiktok : "https://www.tiktok.com/@kbssmoothnutrit?_t=ZT-8suPWQaY1ug&_r=1",
    pinterest : ""
}